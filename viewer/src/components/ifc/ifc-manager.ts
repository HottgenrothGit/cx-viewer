...
export class IfcManager extends IfcComponent {
  ...
  removeAllIfcModels(): void {
    for (let i = 0; i < this.context.items.ifcModels.length; i++) {
      try {
        this.shadowDropper.deleteShadow(i.toString());

        const scene = this.context.getScene();
        
        this.loader.ifcManager.close(i, scene);
      } catch (e) {
        console.log(`Removing IfcModel ${i} failed`);
      }
    }
  }
